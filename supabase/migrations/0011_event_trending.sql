-- Kam denes — 📈 "U trendu" oznaka (ADR-014) treba stvarnu usporedbu
-- nedavnih i prethodnih pregleda, ne samo agregatni popularity_score.
-- Zaseban security definer helper (isti obrazac kao event_popularity_score,
-- 0010) jer bypassira RLS na event_interactions.

create function event_is_trending(p_event_id uuid)
returns boolean
language sql
stable
security definer
...
drop function if exists top_popular_events(integer);
